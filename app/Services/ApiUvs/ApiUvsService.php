...
class ApiUvsService
{
    ...
    /**
     * Erzeugt eine signierte Dokument-URL und prueft, ob darueber wirklich
     * eine lesbare PDF ausgeliefert wird.
     */
    public function testSignedDocument(string $type, string $path, ?string $itemId = null): array
    {
        $payload = [
            'typ' => $type,
            'path' => $path,
        ...
        if ($itemId !== null && trim($itemId) !== '') {
            $payload['item_id'] = trim($itemId);
        }

        $signedResponse = $this->request('POST', '/api/documents/sign', $payload);

        if (!($signedResponse['ok'] ?? false)) {
            return $signedResponse;
        }

        $signedData = $signedResponse['data'] ?? null;
        $signedUrl = is_array($signedData) ? ($signedData['url'] ?? null) : null;

        if (!is_string($signedUrl) || !filter_var($signedUrl, FILTER_VALIDATE_URL)) {
            return [
                'ok' => false,
                'status' => $signedResponse['status'] ?? null,
                'message' => 'Die UVS-API hat keine gueltige signierte Dokument-URL geliefert.',
                'data' => $signedData,
            ];
        }

        if (!$this->hasSameOrigin($signedUrl, $this->baseUrl)) {
            return [
                'ok' => false,
                'status' => $signedResponse['status'] ?? null,
                'message' => 'Die signierte URL verwendet nicht Host, Schema und Port der konfigurierten UVS-API.',
                'data' => [
                    'signed_url' => $signedUrl,
                    'configured_api_url' => $this->baseUrl,
                ],
            ];
        }
        ...
        try {
            // Der PDF-Endpunkt ist absichtlich nur durch die URL-Signatur
            // geschuetzt. Deshalb hier keinen API-Key mitsenden.
            ...
            $hasPdfContentType = str_starts_with($contentType, 'application/pdf');
            $hasPdfHeader = str_starts_with($body, '%PDF-');
            $ok = $response->successful() && $hasPdfContentType && $hasPdfHeader;

            return [
                'ok' => $ok,
                'status' => $response->status(),
                'message' => $ok
                    ? 'Signierte URL erzeugt und PDF erfolgreich abgerufen.'
                    : 'Die signierte URL hat keine gueltige PDF ausgeliefert.',
                'data' => [
                    'url' => $signedUrl,
                    'expires_at' => is_array($signedData) ? ($signedData['expires_at'] ?? null) : null,
                    'sign_status' => $signedResponse['status'] ?? null,
                    'download_status' => $response->status(),
                    'content_type' => $response->header('Content-Type'),
                    'bytes' => strlen($body),
                    'pdf_header_valid' => $hasPdfHeader,
                ],
            ];
        } catch (Throwable $e) {
            Log::error('Signed document test failed', [
                'url' => Str::before($signedUrl, '?') . '?[masked]',
                'error' => $e->getMessage(),
            ]);
    ...
    protected function hasSameOrigin(string $url, string $baseUrl): bool
    {
        $candidate = parse_url($url);
        $configured = parse_url($baseUrl);
        ...
        $configuredPort = (int) ($configured['port'] ?? ($configuredScheme === 'https' ? 443 : 80));

        return $candidatePort === $configuredPort;
    }
}
