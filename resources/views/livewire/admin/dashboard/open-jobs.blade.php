<div class="space-y-3" @if($autoRefresh) wire:poll.120s="loadJobs" @endif>
    <div class="rounded-xl border border-gray-200 bg-gradient-to-b from-gray-50 to-white">
        <div class="divide-y divide-gray-100">
            @forelse($jobs as $job)
                @php
                    $isOverdue = $job->due_at && $job->due_at->isPast();
                @endphp
                   ...
                   class="block px-3 py-2.5 hover:bg-gray-50 transition">
                    <div class="flex items-start justify-between gap-3">
                        ...
                        <span class="shrink-0 inline-flex items-center rounded-full border px-2 py-0.5 text-[11px]
                            {{ $job->status === \App\Models\AdminTask::STATUS_OPEN ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-sky-200 bg-sky-50 text-sky-700' }}">
                            {{ $job->status_text }}
                        </span>
                    </div>
                    ...
                    <div class="mt-1.5 flex items-center justify-between gap-3">
                        <div class="text-[11px] text-gray-500">
                            ...
                            @if($job->assignedAdmin)
                                - Zuständig: {{ $job->assignedAdmin->name }}
                            @endif
                        </div>
                        <div class="text-[11px] {{ $isOverdue ? 'text-red-700 font-semibold' : 'text-gray-500' }}">
                            @if($job->due_at)
                                Fällig: {{ $job->due_at->format('d.m.Y H:i') }}
                            @else
                                Keine Frist
                            @endif
                        </div>
