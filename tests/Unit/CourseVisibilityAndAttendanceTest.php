...
class CourseVisibilityAndAttendanceTest extends TestCase
{
    ...
    public function test_changed_admin_attendance_views_compile(): void
    {
        ...
        $this->assertStringContainsString('runRowAction(action)', $modalSource);
        $this->assertStringNotContainsString('saving: false', $modalSource);
        $this->assertStringContainsString('$wire.clearTimes', $modalSource);
        $this->assertStringContainsString('fad fa-spinner-third fa-spin', $modalSource);
        $this->assertStringContainsString('x-show="rowLoading"', $modalSource);
        $this->assertSame(1, substr_count($modalSource, 'x-show="rowLoading"'));
        $this->assertStringNotContainsString('wire:target="markPresent', $modalSource);
